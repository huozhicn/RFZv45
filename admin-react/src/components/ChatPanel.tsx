    ...
    <div style={{
      ...
      <div style={{
        ...
        <textarea value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={handleKeydown}
          placeholder="输入指令..." disabled={sending} rows={1}
          style={{ flex: 1, padding: '8px 12px', border: '1px solid #d9d9d9', borderRadius: 18, fontSize: 13, resize: 'none', fontFamily: 'inherit', outline: 'none' }}
        ...
        <button onClick={sendMessage} disabled={sending || !inputText.trim()}
          style={{ padding: '8px 18px', background: sending ? '#91caff' : '#1677ff', color: '#fff', border: 'none', borderRadius: 18, fontSize: 13, cursor: sending ? 'not-allowed' : 'pointer', fontWeight: 500, whiteSpace: 'nowrap' }}
        >{sending ? '...' : '发送'}</button>
      </div>
    </div>
  )
